...
struct MyApp {
    ...
    angle_y: f64,
    angle_z: f64,
    last_update: Instant, // Track time for smooth animation
}
...
impl MyApp {
    ...
    fn smooth_update_rotation(&mut self) {
        ...
        match self.rotation_axis.as_str() {
            "x" => self.angle_x += ROTATION_SPEED * delta * 60.0,
            "y" => self.angle_y += ROTATION_SPEED * delta * 60.0,
            "z" => self.angle_z += ROTATION_SPEED * delta * 60.0,
            _ => (),
        }
    }
